    ...
    return (
        <View className='flex-1 flex'>
            ...
            <View className="flex-row justify-center pt-4 space-x-2">
                ...
                >
                    <Text className={`text-xs ${i18n.language === 'en' ? 'text-white font-bold' : 'text-typography-600'}`}>EN</Text>
                </TouchableOpacity>
                <TouchableOpacity
                    onPress={() => changeLanguage('hi')}
                    className={`px-3 py-1 rounded-full ${i18n.language === 'hi' ? 'bg-primary-500' : 'bg-gray-200'}`}
                >
                    <Text className={`text-xs ${i18n.language === 'hi' ? 'text-white font-bold' : 'text-typography-600'}`}>हि</Text>
                </TouchableOpacity>
                <TouchableOpacity
                    onPress={() => changeLanguage('bn')}
                    className={`px-3 py-1 rounded-full ${i18n.language === 'bn' ? 'bg-primary-500' : 'bg-gray-200'}`}
                >
                    <Text className={`text-xs ${i18n.language === 'bn' ? 'text-white font-bold' : 'text-typography-600'}`}>বা</Text>
                </TouchableOpacity>
            </View>
            ...
            <View>
                <View className="flex flex-col items-center mt-10">
                    <Text className="text-4xl text-center text-typography-800 font-bold">
                        {t('welcomeTitle', "Let's Simplify Your Meds.")}
                    </Text>
                    <Text className="text-xl text-typography-600 mt-4 text-center">
