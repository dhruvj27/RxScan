        ...
        <SafeAreaView className="flex-1 bg-gray-50">
            ...
            <ScrollView className="flex-1 px-6" keyboardShouldPersistTaps="always" showsVerticalScrollIndicator={false}>
                <View className="pt-4 pb-2">
                    <View className="flex-row items-center mb-2">
                        ...
                        <View className="flex-1">
                            <Text className="text-2xl font-bold text-typography-900 mb-1">Dietary Restrictions</Text>
                            <Text className="text-typography-600 text-base leading-relaxed">
                ...
                <View className="relative mb-6" style={{ zIndex: 1000 }}>
                    <Text className="text-typography-900 font-semibold mb-3 text-base">Search Restrictions</Text>
                    <View className="relative">
                        <TextInput
                            ...
                            className="border-2 border-gray-200 rounded-2xl px-5 py-4 pr-14 text-typography-900 bg-background-0 font-medium"
                            style={{
                                shadowColor: '#000',
                    ...
                    {showDropdown && (searchText.length > 0 || filteredOptions.length > 0) && (
                        <View
                            className="absolute top-full left-0 right-0 bg-background-0 border-2 border-gray-100 rounded-2xl mt-2"
                            style={{
                                zIndex: 1001,
                                maxHeight: 220,
                                shadowColor: '#000',
                            ...
                            >
                                {filteredOptions.map((option, index) => (
                                    <TouchableOpacity
                                        ...
                                        activeOpacity={0.6}
                                    >
                                        <Text className="text-typography-800 font-medium">{option}</Text>
                                    </TouchableOpacity>
                                ))}
                                ...
                                {filteredOptions.length === 0 && searchText.length > 0 && (
                                    <View className="px-5 py-4">
                                        <Text className="text-typography-500 text-center font-medium">No matching restrictions found</Text>
                                    </View>
                                )}
                            </ScrollView>
                        </View>
                ...
                <View className="mb-6">
                    <Text className="text-typography-900 font-semibold mb-4 text-base">Your Restrictions</Text>
                    <View className="bg-background-0 rounded-2xl p-4 border border-gray-200" style={{
                        shadowColor: '#000',
                        shadowOffset: { width: 0, height: 1 },
                        shadowOpacity: 0.05,
                        shadowRadius: 3,
                        ...
                        <View className="flex-row flex-wrap">
                            ...
                            {selectedRestrictions.length === 0 && (
                                <View className="items-center justify-center w-full py-8">
                                    ...
                                    <Text className="text-typography-500 font-medium text-center">No restrictions selected yet</Text>
                                    <Text className="text-typography-400 text-sm text-center mt-1">Tap above to search and add restrictions</Text>
                                </View>
                            )}
                        </View>
                    </View>
                ...
                <View className="mb-6">
                    <Text className="text-typography-900 font-semibold mb-3 text-base">Additional Health Notes</Text>
                    <Text className="text-typography-600 text-sm mb-3 leading-relaxed">
                        Include any other relevant health information (pregnancy, breastfeeding, recent surgeries, etc.)
                    </Text>
                    <View className="bg-background-0 rounded-2xl border-2 border-gray-200" style={{
                        shadowColor: '#000',
                        shadowOffset: { width: 0, height: 1 },
                        shadowOpacity: 0.05,
                        shadowRadius: 3,
                        ...
                        <TextInput
                            ...
                            multiline
                            numberOfLines={4}
                            className="px-5 py-4 text-typography-900 font-medium"
            ...
            <View className="px-6 pb-6 pt-4 bg-background-0 border-t border-gray-100">
                <TouchableOpacity
                    onPress={handleComplete}
                    disabled={isLoading}
