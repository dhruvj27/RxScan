        ...
        <SafeAreaView className="flex-1 bg-gray-50">
            <ScrollView className="flex-1 px-6" keyboardShouldPersistTaps="always">
                <View className="mt-4 mb-8">
                    <View className="flex-row items-center mb-4">
                        ...
                        <View className="flex-1">
                            <Text className="text-2xl font-bold text-typography-900 mb-1">Current Medications</Text>
                            <Text className="text-typography-500 text-sm leading-5">
                ...
                <TouchableOpacity
                    ...
                    className="bg-background-0 border-2 border-dashed border-emerald-300 rounded-2xl py-8 items-center mb-6 shadow-sm"
                    style={{ elevation: 2 }}
                >
                ...
                <View className="mb-6">
                    <Text className="text-lg font-semibold text-typography-800 mb-3">Current Medications</Text>
                    {medications.map((med, index) => (
                        <View key={index} className="bg-background-0 rounded-2xl p-5 mb-4 shadow-sm border border-gray-100" style={{ elevation: 2 }}>
                            <View className="flex-row justify-between items-start">
                                <View className="flex-1 mr-3">
                                    <View className="flex-row items-center mb-2">
                                        <View className="w-3 h-3 bg-purple-400 rounded-full mr-2" />
                                        <Text className="text-typography-900 font-bold text-lg flex-1">{med.name}</Text>
                                    </View>
                                    <View className="ml-5">
                                        <View className="flex-row items-center mb-2">
                                            <Ionicons name="flask-outline" size={16} color="#6B7280" />
                                            <Text className="text-typography-600 ml-2 text-sm">
                    ...
                    {medications.length === 0 && (
                        <Text className="text-typography-500 italic text-center py-4">No medications added yet</Text>
                    )}
                </View>
            ...
            {/* Bottom Button */}
            <View className="p-6 bg-background-0 elevation">
                <TouchableOpacity
                    onPress={handleNext}
            ...
            <Modal visible={showModal} animationType="slide" transparent>
                <View className="flex-1 bg-black/50 justify-end">
                    <View className="bg-background-0 rounded-t-3xl p-6 max-h-4/5">
                        <View className="flex-row justify-between items-center mb-6">
                            <Text className="text-xl font-bold text-typography-800">Add Medication</Text>
                        ...
                        <ScrollView keyboardShouldPersistTaps="always">
                            {/* Medication Name */}
                            <View className="mb-4">
                                <Text className="text-typography-700 font-medium mb-2">Medication Name *</Text>
                                <View className="relative">
                                    <TextInput
                                        ...
                                        className="border border-gray-200 rounded-xl px-4 py-3 text-typography-800"
                                    />
                                    {activeField === 'name' && (
                                        <ScrollView
                                            className="absolute top-full left-0 right-0 bg-background-0 border border-gray-200 rounded-xl mt-1 max-h-32 z-10"
                                        ...
                                        >
                                            {filteredMedications.map((med, index) => (
                                                ...
                                                >
                                                    <Text className="text-typography-800">{med}</Text>
                                                </TouchableOpacity>
                                            ))}
                                        </ScrollView>
                                    )}
                            ...
                            {/* Dosage */}
                            <View className="mb-4">
                                <Text className="text-typography-700 font-medium mb-2">Dosage</Text>
                                <View className="relative">
                                    <TextInput
                                        value={dosageText}
                                        onChangeText={(text) => {
                                        ...
                                        >
                                            {filteredDosages.map((dosage, index) => (
                                                <TouchableOpacity
                                                    ...
                                                    className="px-4 py-3 border-b border-gray-100"
                                                >
                                                    <Text className="text-typography-800">{dosage}</Text>
                                                </TouchableOpacity>
                                            ))}
                                        </ScrollView>
                                    )}
                            ...
                            {/* Frequency */}
                            <View className="mb-6">
                                <Text className="text-typography-700 font-medium mb-2">Frequency</Text>
                                <View className="relative">
                                    <TextInput
                                        value={frequencyText}
                                        onChangeText={(text) => {
                                        ...
                                        >
                                            {filteredFrequencies.map((freq, index) => (
                                                <TouchableOpacity
                                                    ...
                                                    className="px-4 py-3 border-b border-gray-100"
                                                >
                                                    <Text className="text-typography-800">{freq}</Text>
                                                </TouchableOpacity>
                                            ))}
                                        </ScrollView>
                                    )}
